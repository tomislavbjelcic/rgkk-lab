...
contract Auction {
    ...
    function isAddressDefined(address adr) internal pure returns (bool) {
    ...
    function finalize() public {
        ...
        // nije moguc poziv finalize() ako nije SUCCESSFUL
        require(outcome == Outcome.SUCCESSFUL);

        if (isAddressDefined(judgeAddress)) {
            // ako sudac postoji, provjeri tko poziva finalize()
            require((msg.sender == judgeAddress) || (msg.sender == highestBidderAddress));
        }

        address payable payTo = payable(sellerAddress);
        address self = address(this);
        payTo.transfer(self.balance);
    }
    ...
    function refund() public {
        ...
        // nije moguc poziv refund() ako nije NOT_SUCCESSFUL
        ...
        require(isAddressDefined(highestBidderAddress));

        address payable payTo = payable(highestBidderAddress);
        address self = address(this);
        payTo.transfer(self.balance);
    }
